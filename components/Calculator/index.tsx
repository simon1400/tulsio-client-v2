...
const Calculator: FC<{ embed?: string }> = ({ embed = '' }) => {
  ...
  const [kapek, setKapek] = useState<number>(
    (lavelState / (cbd / pocetKapek)) * ((kg as number) / baseKg),
  )
  ...
  return (
    <CalculatorS embed={embed}>
      ...
      <Typography variant={'h3'}>
        {state === 'looking' ? 'Doporučená koncentrace kapek' : 'Vyplňte údaje z vašich kapek'}
      </Typography>
      <InputsBlockWrap>
        <div>
      ...
      <Grid container>
        <Grid item xs={3}>
          <ResultCalculate animation={animation} delay={0}>
            ...
            <span>
              {((cbd / pocetKapek) * kapek).toFixed(0)}
              {' mg CBD'}
            </span>
          </ResultCalculate>
        </Grid>
